...
public class GlobalExceptionHandler {
    ...
    //URL incompleta en caso de PUT
    @ExceptionHandler(ServletRequestBindingException.class)
public ResponseEntity<ApiError> handleServletRequestBinding(ServletRequestBindingException ex, WebRequest request) {
    ...
    if (metodo.equals("PUT") && url.endsWith("/")) { 
        String mensaje = "La URL está incompleta. Debe incluir el ID del recurso. Ejemplo: PUT /api/.../1";
        
        ApiError apiError = new ApiError(
            HttpStatus.BAD_REQUEST.value(),
            "URL incompleta para PUT",
            mensaje,
            LocalDateTime.now()
        );
